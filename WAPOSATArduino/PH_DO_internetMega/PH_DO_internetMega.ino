...
int Pin_x = 2;                      //Arduino pin 5 to control pin X
int Pin_y = 3;                      //Arduino pin 4 to control pin Y
//-----------------------------------datos de sensor--------------------------------------------------
...
//-----------------configuracion de serial sensor-----------------------------------------------------
  ...
  Serial1.begin(9600);             //Set the soft serial port to 9600
...
void serialEvent1(){
  if (sensor == 1){
  sensor_bytes_received1=Serial1.readBytesUntil(13,sensordata1,30);
  sensordata1[sensor_bytes_received1]=0;
  } else {
  sensor_bytes_received2=Serial1.readBytesUntil(13,sensordata2,30);
  sensordata2[sensor_bytes_received2]=0;
  }
}
...
void httpRequest() {
  ...
  if (client.connect(server, 80)) {
    ...
    client.print("GET /WAPOSAT3/Template/InsertData3.php?equipo=4&sensor1=1&sensor2=2&sensor3=3&valor1="); // Envia los datos utilizando GET
    client.print(SD1);
    client.print("&valor2=");
    client.print(T);
    client.print("&valor3=");
...
void LecturaSensores() {
  //-----------------------sensor de pH------------------------------------------------------------------  
      ...
      delay(1000);
      Serial1.print('r');                      //Send the command from the computer to the Atlas Scientific device using the softserial port                                     
      Serial1.print("\r");     
      delay(500);
      /*
      sensor_bytes_received1=Serial1.readBytesUntil(13,sensordata1,30); //we read the data sent from the Atlas Scientific device until we see a <CR>. We also count how many character have been received 
      ...
      */
      if(sensor_bytes_received1!=0){
      Serial.println(sensordata1);
      sensor_bytes_received1=0;
      } else {
  ...
  //-----------------------sensor de DO------------------------------------------------------------------  
      ...
      delay(500);
      /*
      sensor_bytes_received2=Serial1.readBytesUntil(13,sensordata2,30); //we read the data sent from the Atlas Scientific device until we see a <CR>. We also count how many character have been received 
      ...
      */
      if(sensor_bytes_received2!=0){
      Serial.println(sensordata2);
      sensor_bytes_received2=0;
      } else {
      Serial.println("no encontrado");
      }
      delay(500);
